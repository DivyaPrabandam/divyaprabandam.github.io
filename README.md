...
Build with JDK 17 and SDK 37: `ANDROID_HOME=/tmp/android-sdk JAVA_HOME=/tmp/jdk17 /tmp/gradle-root/gradle-8.13/bin/gradle :app:assembleDebug`. No direct production download link exists. Before one is created: configure the staged signed production publisher and verify live current text, get approval on the updated owner-supplied icon previews and check icon switching on-device, complete corrections phone-path QA if enabling it, run real-device functional/security/battery/RAM checks on Android 12+ including the Xiaomi 15 Android 16 phone, and visually inspect all four themes. Host software emulator could not boot due no KVM, 2 GB RAM and disk requirements. The v6 phone QA confirmed the earlier book-list crash was fixed, not this new binary. Do not merge/release merely because compilation passes.
