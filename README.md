...
The launcher has one icon only: the owner's own transparent PNG of Ramanujar and the deity, enlarged as far as the adaptive round mask safely allows. No in-app switching or second launcher alias. Current preview awaits owner review and phone QA.
...
Build with JDK 17 and SDK 37: `ANDROID_HOME=/tmp/android-sdk JAVA_HOME=/tmp/jdk17 /tmp/gradle-root/gradle-8.13/bin/gradle :app:assembleDebug`. No direct production download link exists. Before one is created: configure the staged signed production publisher and verify live current text, get approval on the updated owner-supplied single-icon previews and check launcher rendering on-device, complete corrections phone-path QA if enabling it, run real-device functional/security/battery/RAM checks on Android 12+ including the Xiaomi 15 Android 16 phone, and visually inspect all four themes. Host software emulator could not boot due no KVM, 2 GB RAM and disk requirements. The v6 phone QA confirmed the earlier book-list crash was fixed, not this new binary. Do not merge/release merely because compilation passes.
