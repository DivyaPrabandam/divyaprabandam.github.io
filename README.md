...
Verification so far: Gradle debug build passes, manifest identifies SDK floor; manifest validation finds all 4,000 numbers once with no gaps. Real Android UI screenshot and interactive QA are still required before an installable alpha can be offered for use. Software emulators here are blocked by lack of KVM / low host memory.

## Local next-step branch (not in the committed alpha)

After the first pinned alpha was handed off, a second, uncommitted slice added:
