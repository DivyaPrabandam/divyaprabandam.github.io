...
final class SpeedDial extends View {
    ...
    private float value;
    SpeedDial(Context context,float initial,Change change){super(context);this.value=initial;this.change=change;
        setMinimumWidth(dp(240));setMinimumHeight(dp(260));setContentDescription("Rotate to adjust playback speed");
        GradientDrawable surface=new GradientDrawable();surface.setColor(0xff20160f);surface.setCornerRadius(dp(14));setBackground(surface);}
    ...
    @Override protected void onDraw(Canvas canvas){
        ...
        paint.setColor(0xff38291d);canvas.drawArc(cx-r,cy-r,cx+r,cy+r,135,270,false,paint);
        paint.setColor(0xffffb74d);float progress=(value-.5f)/1.5f;
        ...
        paint.setColor(0xffffffff);paint.setTextAlign(Paint.Align.CENTER);paint.setTextSize(dp(36));paint.setFakeBoldText(true);
        canvas.drawText(String.format(java.util.Locale.ROOT,"%.2f×",value),cx,cy+dp(10),paint);
        paint.setFakeBoldText(false);paint.setTextSize(dp(13));paint.setColor(0xffbbbbbb);
